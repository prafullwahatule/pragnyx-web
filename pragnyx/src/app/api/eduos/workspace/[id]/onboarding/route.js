...
import { getWorkspace, updateWorkspaceInstitution, updateWorkspaceAddOns, completeOnboarding } from "@/lib/repo/eduos";
import { getCustomerSession } from "@/lib/eduos/session";
import { createSessionToken, sessionCookieOptions } from "@/lib/eduos/auth";
import { sendTeamInviteEmail } from "@/lib/eduos/emails";

// Only these fields can be patched from the onboarding wizard's "confirm
// institution details" step — same shape SignupWizard collects at signup,
// deliberately not a free-form pass-through of the request body.
const INSTITUTION_FIELDS = ["name", "type", "contactPerson", "email", "phone", "students", "staff", "address"];

function pickInstitutionPatch(input) {
  const patch = {};
 ...
 * (src/components/eduos/OnboardingWizard.jsx). Called once, either at
 ...
 *
 * Falls under the same "/api/eduos/workspace/" middleware prefix as the
 * addons route, so it's already gated on being logged in AND having
...
export async function POST(request, { params }) {
  ...
  if (!body?.skip) {
    if (body?.institution && typeof body.institution === "object") {
      await updateWorkspaceInstitution(decodedId, pickInstitutionPatch(body.institution));
    }
    ...
    if (invite?.email && invite?.name) {
      try {
        ...
        console.error("[eduos] team invite email failed", err);
      }
    }
  }
