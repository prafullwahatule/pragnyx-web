...
import { verifyWebhookSignature } from "@/lib/fincore/razorpay";
import { getWorkspaceByOrderId, updateWorkspaceSubscriptionStatus } from "@/lib/repo/fincore";
import { sendReceiptEmail } from "@/lib/fincore/emails";
import { wasEventProcessed, markEventProcessed } from "@/lib/webhookIdempotency";

// Razorpay webhook — configure this URL (https://pragnyx.in/api/fincore/webhook)
...
export async function POST(request) {
  ...
  switch (type) {
    ...
    case "order.paid": {
      ...
      if (!workspace) {
        console.log("[fincore] webhook: no workspace found for order (likely the initial signup payment, already handled by /checkout/verify)", { orderId, type });
        break;
      }
      ...
      const updated = await updateWorkspaceSubscriptionStatus(workspace.companyId, {
        status: "active",
        renewalDate,
      ...
      try {
        ...
        console.error("[fincore] receipt email failed", err);
      }
      break;
    }
    ...
    case "payment.failed": {
      ...
      if (!workspace) {
        console.log("[fincore] webhook: payment.failed for an order with no matching workspace", { orderId });
        break;
      }

      // Grace period, not immediate suspension — the customer keeps
      // access while they retry payment or sort out a card decline.
      await updateWorkspaceSubscriptionStatus(workspace.companyId, { status: "past_due" });
      break;
    }
    ...
    case "refund.processed": {
      ...
      if (!workspace) {
        console.log("[fincore] webhook: refund.processed for an order with no matching workspace", { orderId });
        break;
      }
      ...
      await updateWorkspaceSubscriptionStatus(workspace.companyId, {
        status: isFullRefund ? "cancelled" : "suspended",
      });
      console.log("[fincore] refund processed", {
    ...
    default:
      console.log("[fincore] webhook: unhandled event type", { type });
      break;
  }
