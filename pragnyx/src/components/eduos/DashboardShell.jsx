...
import { ADD_ONS, PLANS as STATIC_PLANS } from "@/lib/eduos/plans";
...
export default function DashboardShell({ workspaceId }) {
  ...
  async function handleLogout() {
    setLoggingOut(true);
    try {
      await fetch("/api/eduos/logout", { method: "POST" });
    } finally {
      router.push("/eduos/login");
      router.refresh();
    }
  }

  useEffect(() => {
    fetch("/api/eduos/plans")
...
function Modules({ workspace, workspaceId, addOnCatalogue, onAddOnsChange }) {
  ...
  async function toggleAddOn(addonId, isActive) {
    ...
    setPending(addonId);
    try {
      const res = await fetch(`/api/eduos/workspace/${encodeURIComponent(workspaceId)}/addons`, {
        method: "POST",
