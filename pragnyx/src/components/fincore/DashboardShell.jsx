    ...
    return (
      <div className="e-card" style={{ padding: 40, maxWidth: 480, margin: "60px auto", textAlign: "center" }}>
        ...
        <p style={{ marginTop: 10, fontSize: 14, color: "var(--e-ink-mute)" }}>
          Log in with your workspace URL or company code to see your dashboard.
        </p>
        <Link href="/fincore/login" className="e-btn e-btn-primary" style={{ marginTop: 20, display: "inline-flex" }}>
          Go to login
        </Link>
      </div>
    );
    ...
    <>
      <Card title="Workspace">
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px 20px" }} className="e-wizard-2col">
          ...
          <Field label="Company ID" value={workspace.companyId} />
      ...
      <Card title="Usage limits">
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px 20px" }} className="e-wizard-2col">
          <Field label="Company limit" value={workspace.limits.companies} />
          <Field label="User limit" value={workspace.limits.users} />
          ...
          <Field label="Invoice limit / month" value={workspace.limits.invoices} />
        </div>
      </Card>
    </>
  );
...
function Team({ workspace }) {
  const [members, setMembers] = useState([
    { name: workspace.business.name, email: workspace.admin.email, role: "Super Admin" },
  ]);
        ...
        <form onSubmit={addMember} style={{ display: "grid", gridTemplateColumns: "1fr 1fr auto auto", gap: 10, alignItems: "end" }} className="e-invite-form">
          ...
          <div className="e-field">
            ...
            <select className="e-select" value={invite.role} onChange={(e) => setInvite((v) => ({ ...v, role: e.target.value }))}>
              {["Admin", "Accountant", "Finance Manager", "Sales", "Inventory Manager", "Viewer"].map((r) => <option key={r} value={r}>{r}</option>)}
            </select>
          </div>
