...
export default function FinCoreNavbar() {
  ...
  return (
    ...
    >
      <nav className="e-shell" style={{ height: 76, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <Link href="/fincore" style={{ display: "flex", alignItems: "center", gap: 9 }} onClick={() => setOpen(false)}>
          <span
            style={{
              width: 32,
              height: 32,
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <ThemeToggle />
          <Link href="/fincore/login" className="e-btn e-btn-ghost e-btn-sm e-hide-mobile">
            Log in
          </Link>
          <Link href="/fincore/pricing" className="e-btn e-btn-primary e-btn-sm e-hide-mobile">
            Get Started
          </Link>
          <button
            onClick={() => setOpen((v) => !v)}
      ...
      {open && (
        <div style={{ borderTop: "1px solid var(--e-border)", padding: "20px 24px", display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <Link href="/fincore/login" onClick={() => setOpen(false)} className="e-btn e-btn-ghost e-btn-block">
            Log in
          </Link>
          <Link href="/fincore/pricing" onClick={() => setOpen(false)} className="e-btn e-btn-primary e-btn-block">
            Get Started
          </Link>
        </div>
      )}
