...
import { ADD_ONS as STATIC_ADD_ONS } from "@/lib/fincore/plans";
...
const ROLES = ["Admin", "Accountant", "Finance Manager", "Sales", "Inventory Manager", "Viewer"];
 ...
 * calls are what flip that flag (see /api/fincore/workspace/[id]/onboarding).
 *
...
export default function OnboardingWizard({ workspaceId }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/fincore/workspace/${encodeURIComponent(workspaceId)}`)
      .then((r) => (r.ok ? r.json() : Promise.reject()))
      .then((data) => {
        setBusiness((v) => ({ ...v, ...data.workspace.business }));
  ...
  async function finish({ skip = false, includeInvite = true } = {}) {
    ...
    setSubmitError("");
    try {
      const res = await fetch(`/api/fincore/workspace/${encodeURIComponent(workspaceId)}/onboarding`, {
        method: "POST",
    ...
    <div style={{ maxWidth: 720, margin: "0 auto" }}>
      ...
      {step === "details" && (
        <form onSubmit={handleDetailsSubmit} className="e-card" style={{ padding: 32, display: "flex", flexDirection: "column", gap: 16, opacity: loadingWorkspace ? 0.6 : 1 }}>
          <div>
            <h2 style={{ fontSize: 20, fontWeight: 600 }}>Confirm your business details</h2>
          ...
          <div className="e-field">
            <label className="e-label" htmlFor="o-name">Company name</label>
            <input id="o-name" className="e-input" value={business.name} onChange={update("name")} />
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }} className="e-wizard-2col">
            <div className="e-field">
              <label className="e-label" htmlFor="o-type">Business type</label>
              <select id="o-type" className="e-select" value={business.type} onChange={update("type")}>
            ...
            <div className="e-field">
              <label className="e-label" htmlFor="o-industry">Industry</label>
              <select id="o-industry" className="e-select" value={business.industry} onChange={update("industry")}>
                <option value="" disabled>Select one</option>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }} className="e-wizard-2col">
            <div className="e-field">
              <label className="e-label" htmlFor="o-gst">GST number</label>
              <input id="o-gst" className="e-input" value={business.gstNumber} onChange={update("gstNumber")} />
            </div>
            <div className="e-field">
              <label className="e-label" htmlFor="o-pan">PAN</label>
              <input id="o-pan" className="e-input" value={business.pan} onChange={update("pan")} />
            </div>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }} className="e-wizard-2col">
            <div className="e-field">
              ...
              <input id="o-email" type="email" className="e-input" value={business.email} onChange={update("email")} />
              {errors.email && <span className="e-error">{errors.email}</span>}
            </div>
            <div className="e-field">
              <label className="e-label" htmlFor="o-phone">Phone</label>
              <input id="o-phone" className="e-input" value={business.phone} onChange={update("phone")} />
              {errors.phone && <span className="e-error">{errors.phone}</span>}
            </div>
          </div>

          <div className="e-field">
            <label className="e-label" htmlFor="o-employees">Number of employees</label>
            <input id="o-employees" type="number" min="0" className="e-input" value={business.employees} onChange={update("employees")} />
          </div>

          <div className="e-field">
            <label className="e-label" htmlFor="o-address">Address</label>
            <textarea id="o-address" className="e-textarea" rows={2} value={business.address} onChange={update("address")} />
          </div>
