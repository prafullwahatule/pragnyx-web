...
export default function SignupWizard({ initialPlan }) {
  ...
  const [business, setBusiness] = useState({
    name: "", type: "", gstNumber: "", pan: "", email: "", phone: "", industry: "", employees: "", address: "",
  });
  ...
  function validateDetails() {
    const errs = {};
    if (!business.name.trim()) errs.name = "Company name is required.";
    if (!business.type) errs.type = "Select a business type.";
    if (!business.email.trim() || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(business.email)) errs.email = "A valid email is required.";
    if (!business.phone.trim()) errs.phone = "Phone number is required.";
    setErrors(errs);
      ...
      try {
        await fetch("/api/fincore/demo", {
          ...
          body: JSON.stringify({
            ...
            businessSize: `${business.employees || "?"} employees`,
            message: `Enterprise plan interest via signup flow. Type: ${business.type}. Industry: ${business.industry}. Address: ${business.address}`,
          }),
        });
        setDemoSent(true);
        setStep("success");
  ...
  async function startPayment() {
    ...
    setPayError("");
    try {
      const orderRes = await fetch("/api/fincore/checkout/create-order", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      const rzp = new window.Razorpay({
        ...
        handler: async (response) => {
          try {
            const verifyRes = await fetch("/api/fincore/checkout/verify", {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({
  ...
  return (
    <div style={{ maxWidth: 720, margin: "0 auto" }}>
      ...
      {step === "details" && (
        <form onSubmit={handleDetailsSubmit} className="e-card" style={{ padding: 32, display: "flex", flexDirection: "column", gap: 16 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
            <h2 style={{ fontSize: 20, fontWeight: 600 }}>Business details</h2>
            <span style={{ fontSize: 12.5, color: "var(--e-ink-faint)" }}>Plan: {plan.name}</span>
          </div>

          <div className="e-field">
            <label className="e-label" htmlFor="i-name">Company name</label>
            <input id="i-name" className="e-input" value={business.name} onChange={update("name")} />
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }} className="e-wizard-2col">
            <div className="e-field">
              <label className="e-label" htmlFor="i-type">Business type</label>
              <select id="i-type" className="e-select" value={business.type} onChange={update("type")}>
            ...
            <div className="e-field">
              <label className="e-label" htmlFor="i-industry">Industry</label>
              <select id="i-industry" className="e-select" value={business.industry} onChange={update("industry")}>
                <option value="" disabled>Select one</option>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }} className="e-wizard-2col">
            <div className="e-field">
              <label className="e-label" htmlFor="i-gst">GST number</label>
              <input id="i-gst" className="e-input" value={business.gstNumber} onChange={update("gstNumber")} />
            </div>
            <div className="e-field">
              <label className="e-label" htmlFor="i-pan">PAN</label>
              <input id="i-pan" className="e-input" value={business.pan} onChange={update("pan")} />
            </div>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }} className="e-wizard-2col">
            <div className="e-field">
              ...
              <input id="i-email" type="email" className="e-input" value={business.email} onChange={update("email")} />
              {errors.email && <span className="e-error">{errors.email}</span>}
            </div>
            <div className="e-field">
              <label className="e-label" htmlFor="i-phone">Phone</label>
              <input id="i-phone" className="e-input" value={business.phone} onChange={update("phone")} />
              {errors.phone && <span className="e-error">{errors.phone}</span>}
            </div>
          </div>

          <div className="e-field">
            <label className="e-label" htmlFor="i-employees">Number of employees</label>
            <input id="i-employees" type="number" min="0" className="e-input" value={business.employees} onChange={update("employees")} />
          </div>

          <div className="e-field">
            <label className="e-label" htmlFor="i-address">Address</label>
            <textarea id="i-address" className="e-textarea" rows={2} value={business.address} onChange={update("address")} />
          </div>
      ...
      {step === "success" && (
        <SuccessPanel workspace={workspace} demoSent={demoSent} plan={plan} business={business} />
      )}
...
function SuccessPanel({ workspace, demoSent, plan, business }) {
    ...
    return (
      <div className="e-card" style={{ padding: 40, textAlign: "center" }}>
        ...
        <p style={{ marginTop: 10, fontSize: 14.5, color: "var(--e-ink-mute)", maxWidth: 420, margin: "10px auto 0" }}>
          Our team will reach out to {business.email} within one business day to scope your Enterprise rollout.
        </p>
        <Link href="/fincore" className="e-btn e-btn-ghost" style={{ marginTop: 24, display: "inline-flex" }}>
          Back to FinCore
        </Link>
      </div>
    );
  ...
  return (
    <div className="e-card" style={{ padding: 36 }}>
      <div style={{ textAlign: "center" }}>
        ...
        <p style={{ marginTop: 8, fontSize: 14, color: "var(--e-ink-mute)" }}>
          {business.name} is live on the {plan.name} plan. Credentials below have also been emailed to {workspace.admin.email}.
        </p>
      </div>
      ...
      <div style={{ marginTop: 28, display: "flex", gap: 12, flexWrap: "wrap", justifyContent: "center" }}>
        ...
        <Link href={`/fincore/login?workspace=${encodeURIComponent(workspace.companyId)}`} className="e-btn e-btn-primary">
