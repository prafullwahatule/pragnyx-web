...
// src/lib/eduos/plans.js so a missing row just falls back silently.

const globalPrices = globalThis;
if (!globalPrices.__eduosPlanPrices) globalPrices.__eduosPlanPrices = new Map();
const memPlanPrices = globalPrices.__eduosPlanPrices;
...
export async function getPlanPriceOverrides() {
  ...
  await ensureSchema();
  const res = await query("SELECT plan_id, price FROM eduos_plan_prices");
  const map = {};
...
export async function setPlanPrice(planId, price) {
  ...
  await ensureSchema();
  await query(
    `INSERT INTO eduos_plan_prices (plan_id, price, updated_at)
     VALUES ($1, $2, now())
